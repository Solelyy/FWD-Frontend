...
import { CashAdvanceActionProps } from "../../cash-advance/types/ca-actions";
import { getActionsByStatus } from "../types/actions";
import { useReimbursementActions } from "../hooks/useEmployeesActions";

import ReimbursementDialog from "./ReimbursementDialog";

type Props = {
    request: EmployeeReimbursementRequest
}
export default function ReimbursementActions({request}: Props) {
    const [open, setOpen] = useState(false);
    const [selectedAction, setSelectedAction] = useState<CashAdvanceActionProps | null>(null);
    
    const { mutate, isPending } = useReimbursementActions();
