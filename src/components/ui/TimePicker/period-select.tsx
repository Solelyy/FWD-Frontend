...
export const TimePeriodSelect = React.forwardRef<HTMLButtonElement, PeriodSelectorProps>(({ period, setPeriod, date, setDate, onLeftFocus, onRightFocus }, ref) => {
    ...
    const handleValueChange = (value: Period) => {
         ...
         */
        const baseDate = date ?? new Date(new Date().setHours(0, 0, 0, 0));
        const tempDate = new Date(baseDate);
        const hours = display12HourValue(baseDate.getHours());
    ...
    return (
        <div className="flex h-10 items-center">
            <Select value={period} onValueChange={(value: Period) => handleValueChange(value)}>
                <SelectTrigger ref={ref} className="h-10 w-[72px] border-border/70 bg-background focus:bg-accent/70 focus:text-accent-foreground" onKeyDown={handleKeyDown}>
                    <SelectValue />
                </SelectTrigger>
                <SelectContent>
